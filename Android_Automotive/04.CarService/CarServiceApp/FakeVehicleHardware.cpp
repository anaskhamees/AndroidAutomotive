    ...
    *isSpecialValue = false;
    ...
    switch (propId) {
     ...
     case toInt(TestVendorProperty::LED): {
        ...
        /*case toInt(TestVendorProperty::gpio): {
            GpioHal gpio;  
            bool value;  // Declare a variable to store the GPIO value
            ALOGD("readvalue");
            bool state = gpio.getGpioValue(2, &value);
            ALOGD("readvalue + ")     ;   
            result = mValuePool->obtainInt32(value);
            return result;  // Return the status of the GPIO read operation
        }

        case toInt(TestVendorProperty::prob): {
            ALOGD("readvalue");
            
            return {} ; // Return the status of the GPIO read operation
        }


        case toInt(TestVendorProperty::i2c): {
            *isSpecialValue = true;
            int rpm = getRpm();  // Call your function to retrieve the RPM value
            result = mValuePool->obtainInt32(rpm);
            ALOGD("RPM %d",rpm);
...
VhalResult<void> FakeVehicleHardware::maybeSetSpecialValue(const VehiclePropValue& value,
    ...
    *isSpecialValue = false;
    ...
    case toInt(TestVendorProperty::LED):
    {
    	GPIO_HAL gpio;
    	int gpioPin=17;
    	gpio.exportGPIO(gpioPin);
    	gpio.setGPIODirection(gpioPin,"out");
    	...
    	/*case toInt(TestVendorProperty::ITI):
            ALOGD("iti property");
            return {};
        
        case toInt(TestVendorProperty::gpio):
            ALOGD("gpio property start");
            GpioHal gpio;
            ALOGD("gpio property start");
            if(gpio.exportGpio(2)){
                ALOGD("gpio property export");
            }else{
                ALOGD("gpio property not export");
               
            }
            
            gpio.setGpioDirection(2,"out");
            ALOGD("gpio property dir");
            gpio.setGpioValue(2 , 1);
            return {};*/
...
// bool ItoCHal::getADCValue(int channel, int *value) {
